...
- \`horizontal-tb\`는 \`text\`의 가로쓰기를 뜻하며, 텍스트는 왼쪽에서 오른쪽으로, 그리고 위에서 아래로 흐릅니다.
...
- \`text-align\`: 텍스트 정렬을 수직 및 수평으로 조정하는 속성으로, \`writing-mode\`와 함께 사용할 때 텍스트의 전체적인 배치를 더욱 세밀하게 제어할 수 있습니다.
  
  \`\`\`css
  text-align: center;
  \`\`\`

- \`line-height\`: 줄 간격을 조정하여 \`writing-mode\`로 설정한 텍스트의 가독성을 높일 수 있습니다.
  
  \`\`\`css
  line-height: 1.5;
  \`\`\`

- \`direction\`: 텍스트의 흐름 방향을 설정하며, \`writing-mode\`와 조합하여 텍스트의 읽기 방향을 결정합니다. 예를 들어, 아랍어와 같은 언어는 오른쪽에서 왼쪽으로 읽는 것이 일반적이므로 \`direction: rtl;\`과 함께 사용할 수 있습니다.
...
\`writing-mode\` 속성은 다양한 텍스트 레이아웃과 다국어 지원이 필요한 상황에서 매우 유용한 CSS 속성입니다. 기본적으로는 가로쓰기 레이아웃이지만, 세로쓰기가 필요한 경우나 독특한 텍스트 배치가 필요한 디자인에서는 필수적인 역할을 합니다.`;export{n as default};
