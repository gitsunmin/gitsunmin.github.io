...
export const LocalStorage = {
    get: (key: keyof LocalStorageInUse) => {
        ...
        if (value === null) return DefaultLocalStorage[key];
            ...
            console.warn(`Invalid value for ${key}: ${value}. Expected one of ${LOCAL_STORAGE_TYPES[key].join(', ')}`);
            return value;
        }
    },
    set: (key: keyof LocalStorageInUse, value: LocalStorageInUse[keyof LocalStorageInUse]) => {
        if (LOCAL_STORAGE_TYPES[key].includes(value)) {
            localStorage.setItem(key, value);
        } else console.warn(`Invalid value for ${key}: ${value}. Expected one of ${LOCAL_STORAGE_TYPES[key].join(', ')}`);
    },
}
