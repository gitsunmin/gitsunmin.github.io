    ...
    components={{
      ...
      a: (props) => match(props.href)
        .with(P.string.startsWith('/'), (href) => (
          <Link {...props} href={'/til' + href.replace('.mdx', '').replace(/\/index$/, '')} to={'/'} className="text-blue-400" />
        ))
      ...
      img: (props) => <img {...props} src={'' + props.src?.split('/').pop()} className="rounded-lg shadow-md my-4" />,
      pre: (el) => {
