---
title: "[Flutter] 같은 코드를 다르게 빌드하는 방법 (Flavor)"
description: Flutter에서 하나의 코드로 환경별 분리하여 다르게 빌드하는 방법인 Flavor을 적용하는 방법을 소개합니다.
...
6. info탭을 눌러주세요.
7. `Bundle display name`을 `$(APP_NAME)`으로 설정해주세요.
8. `Flvaor`이라는 키를 추가하고, `$(APP_FLAVOR)`로 설정해주세요.
...
이렇게 받아온 값을 이용하여 분기를 할 수 있습니다. 또한 앱에 각 환경에 맞는 라벨이 보이도록 할 수 있습니다. 이 패키지를 사용해 보시길 추천합니다. -> [flutter_flavor](https://pub.dev/packages/flutter_flavor "https://pub.dev/packages/flutter_flavor")
