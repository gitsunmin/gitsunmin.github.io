...
graph TD
    ...
    B[파싱 - Parsing]
    C[추상 구문 트리 - AST 생성]
    D[바이트코드 생성 - Ignition 컴파일러]
    E[바이트코드 실행 및 프로파일링 데이터 수집 - Ignition 인터프리터]
    G{핫스팟 감지 - Hotspot Detection}
    H[최적화 컴파일 - Turbofan]
    I[최적화된 머신 코드 실행 - Optimized Machine Code Execution]
    J[프로그램 종료 - Program End]
...
graph TD
    A[웹어셈블리 모듈 - .wasm 파일]
    B[로딩 및 검증 - Loading and Validation]
    C[빠른 컴파일 - Liftoff]
    D[초기 실행 - Initial Execution]
    E[프로파일링 및 핫스팟 감지 - Profiling and Hotspot Detection]
    F[최적화 컴파일 - Turbofan 최적화된 머신 코드]
    G[최적화된 머신 코드 실행 - Optimized Machine Code Execution]
    H[프로그램 종료 - Program End]
