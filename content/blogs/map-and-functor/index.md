...
image: /images/blogs/functional_programing_thumbnail.webp
author: 'Gitsunmin'
categories:
  - '2023'
tags:
...
이 `어떠한_것`은 위에서 이야기 했듯이, Javascript의 Array와 같이 map을 사용할 수 있는 Mappable한 Class입니다. 함수형프로그래밍에서는 이 Class를 `Functor`라고 부릅니다.

> Functor는 원본 값을 가질 수 있으며, 함수를 입력 받아서 원본 값을 변형하고 새로운 Functor를 반환하는 구조를 의미합니다. 중요한 것은 Functor의 구조가 변하지 않으면서 내부의 값이 함수에 의해 변형된다는 점입니다.

다음에는 Functor라는 개념을 더 학습해서 블로그 글을 작성해보려고 합니다. 정리가 되면 개시하겠습니다. 다만, 아래에 Functor와 함수형 프로그래밍을 사용할 수 있도록 도와주는 간단한 라이브러리인 ts-belt를 소개합니다.
